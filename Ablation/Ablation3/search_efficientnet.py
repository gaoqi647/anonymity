...
# efficientnet
param = {'Architecture': [0.961021518438999]
    , 'Normalization': [0.64]
    , "Convolutions": [0.090163934]
    , "Skip_Connections": [0]
    , "Activation_Functions": [0.005859375]
    , "Pooling_Operations": [0.0,
                             ...
                             0.9523809523809523,
                             1.0]
    , "Regularization": [0.00386100386100386]
    , "Data_Augmentagion": [0.269892564802182]
...
def search_best(param):
    xp, yp = bayesian_optimisation(n_iters=500,
                                   ...
                                   a=param['a'],
                                   b=param['b'])
    pd.DataFrame(xp, yp).to_csv('result/efficientnet/' + str(round(param['a'], 2)) + '_' + str(round(param['b'], 2)) + '.csv')
    tmp1 = max(yp)
    tmp1 = tmp1 * 10000
    tmp2 = 500 - len(yp)
    return -tmp1 - tmp2
