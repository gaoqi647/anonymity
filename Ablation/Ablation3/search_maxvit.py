...
# maxvit
param = {'Architecture': [0.161836390128491]
    , 'Normalization': [0.08]
    , "Convolutions": [0.0655737704918032]
    ...
    , "Pooling_Operations": [0.0,
                             ...
                             0.9523809523809523,
                             1.0]
    , "Regularization": [0.00694980694980694]
    , "Data_Augmentagion": [0.27862593792633]
    ...
    , "Framwork": [0.5]
    , "paraeters": [0.428907014]
    , "batch_size": np.array(batch_size)
    , "learning_rate": [-2.94295955]
...
# 0.185185185 4090
# 删除前馈神经网络、正则化、数据增强
param_grid = np.array([[Architecture, Convolutions, Skip_Connections,
                        Activation_Functions, Pooling_Operations
                           , Regularization, Learning_Rate_Schedules,
                        Training_Algorithm, Output_Functions, train_size, number_label, cos, Input_Size,
                        Framwork,
                        paraeters, batch_size, learning_rate, epochs, Hardware, Number]
                       for Architecture in param['Architecture'] for Convolutions in param['Convolutions']
                       for Skip_Connections in param['Skip_Connections']
                       for Activation_Functions in param['Activation_Functions'] for Pooling_Operations in
                       param['Pooling_Operations']
                       for Regularization in param['Regularization']
                       for Learning_Rate_Schedules in
                       param['Learning_Rate_Schedules']
                       for Training_Algorithm in param['Training_Algorithm'] for Output_Functions in
                       param['Output_Functions']
                       for train_size in param['train_size']
                       for number_label in param['number_label'] for cos in param['cos']
...
data.drop(['Feedforward Networks', 'Normalization', 'Data Augmentagion'], axis=1, inplace=True)
features = data.values
...
def search_best(param):
    xp, yp = bayesian_optimisation(n_iters=500,
                                   ...
                                   a=param['a'],
                                   b=param['b'])
    pd.DataFrame(xp, yp).to_csv('result/maxvit/' + str(round(param['a'], 2)) + '_' + str(round(param['b'], 2)) + '.csv')
    tmp1 = max(yp)
    tmp1 = tmp1 * 10000
    tmp2 = 500 - len(yp)
    return -tmp1 - tmp2
